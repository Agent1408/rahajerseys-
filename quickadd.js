...
function openQuickAdd(cfg) {
  _quickAddCtx = cfg;
  var qaImg = document.getElementById('qa-img');
  qaImg.src = cfg.img || 'https://placehold.co/200x266/0d5c2e/f5c518?text=Jersey';
  qaImg.onerror = function () {
    qaImg.onerror = null;
    qaImg.src = cfg.fallback || 'https://placehold.co/200x266/0d5c2e/f5c518?text=Jersey';
  };
  ...
  if (cfg.presetKit) {
    // Kit was already chosen via the Home/Away toggle on the page — show it as a fixed value
    kitWrap.style.display = '';
    kitSel.innerHTML = '<option value="' + cfg.presetKit + '">' + cfg.presetKit + '</option>';
  } else if (cfg.hasKitType) {
    kitWrap.style.display = '';
  ...
  verSel.innerHTML = '';
  (cfg.versions || ['Fan Version']).forEach(function (v) {
...
function submitQuickAdd() {
  ...
  var item = {
    team: cfg.team,
    kitType: cfg.presetKit || (cfg.hasKitType ? document.getElementById('qa-kit').value : ''),
